...
/***/ function(module, exports, __webpack_require__) {
	...
	var _donut = __webpack_require__(4);

	var d3 = __webpack_require__(2);

	module.exports = {
	  vis: function vis(conf) {

	    var svgObject = (0, _svg.svg)(conf);

	    console.log(svgObject);

	    if (conf.type === 'line') {
	      (0, _line.line)(svgObject, conf);
	    }
	    if (conf.type === 'donut') {
	      (0, _donut.donut)(svgObject, conf);
	    }
	  }
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	function svg(config) {
	    ...
	    var svg = d3.select("#" + config.elementId).append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).style("background", config.styles.svg.background ? config.styles.svg.background : "white").append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.line = line;
	function line(svg, config) {

	    var defs = svg.svg.append("defs");
	    ...
	    // Add the X Axis
	    svg.svg.append("g").attr("class", "axis").attr("transform", "translate(0," + svg.height + ")").call(d3.axisBottom(x));

	    // Add the Y Axis
	    svg.svg.append("g").attr("class", "axis").call(d3.axisLeft(y));

	    svg.svg.selectAll(".axis").selectAll("path").style("stroke", "rgba(0,0,0,0)");

	    svg.svg.selectAll(".axis").selectAll("line").style("stroke", "rgba(0,0,0,0)");

	    svg.svg.selectAll(".axis").selectAll("text").style("font-weight", "700").style("font-family", "Roboto").style("fill", "white");

	    // add the Y gridlines
	    svg.svg.append("g").attr("class", "grid").call(make_y_gridlines().tickSize(-svg.width).tickFormat(""));

	    // define the line
	    var valueline = d3.line().curve(d3.curveNatural).x(function (d) {
	        return x(d.x);
	    }).y(function (d) {
	        return y(d.y);
	    });
	    ...
	    // define the area
	    var area = d3.area().curve(d3.curveNatural).x(function (d) {
	        return x(d.x);
	    }).y0(svg.height).y1(function (d) {
	        return y(d.y);
	    });

	    // Scale the range of the config.values
	    x.domain(d3.extent(config.values, function (d) {
	        return d.x;
	    }));
	    y.domain([0, d3.max(config.values, function (d) {
	        return d.y;
	    })]);

	    // Add the valueline path.
	    svg.svg.append("path").data([config.values]).style("fill", "none").style("stroke", config.styles.line.color ? config.styles.line.color : "black").style("stroke-width", 1).style("filter", "url(#drop-shadow)").attr("d", valueline);

	    svg.svg.append("path").datum(config.values).style("fill", config.styles.line.background ? config.styles.line.background : "black").attr("d", area).style("filter", "url(#drop-shadow)");
	    ...
	    // Add the scatterplot
	    svg.svg.selectAll("dot").data(config.values).enter().append("circle").attr("r", 5).attr("cx", function (d) {
	        return x(d.x);
	    }).attr("cy", function (d) {
	        return y(d.y);
	    }).style("stroke-width", 1).style("stroke", config.styles.line.color ? config.styles.line.color : "black").style("fill", config.styles.line.color ? config.styles.line.color : "black");
	}
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.donut = donut;
	function donut(svg, config) {

	    d3.select("#" + config.elementId).select("svg").select("g").attr("transform", "translate(" + svg.width / 2 + "," + svg.height / 2 + ")");
	    ...
	    var arc = d3.arc().innerRadius(radius - donutWidth) // UPDATED
	    .outerRadius(radius);
	    ...
	    var path = svg.svg.selectAll('path').data(pie(config.values)).enter().append('path').attr('d', arc).style("filter", "url(#drop-shadow)").attr('fill', function (d, i) {
